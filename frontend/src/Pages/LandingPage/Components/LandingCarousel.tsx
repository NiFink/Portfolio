...
function LandingCarousel() {
  ...
  return (
    <div className="overflow-hidden relative h-full ">
      <div
        className="flex h-full transition ease-out duration-500 bg-black"
        style={{ transform: `translateX(-${current * 100}%)` }}
      >
        {slides.map((s, index) => (
          <div key={index} className="flex-shrink-0 w-full h-full bg-black">
            <img src={s} alt={`Slide ${index}`} className="w-full bg-black h-full object-cover rounded-lg" />
          </div>
        ))}
      </div>
      ...
      <div className="absolute bottom-0 py-4 flex justify-center gap-5 w-full">
        {slides.map((s, i) => {
          return <div key={"circle" + i} className={`rounded-full w-5 h-5 ${i === current ? "bg-white" : "bg-gray-500"} cursor-pointer`} onClick={ () => setCurrent(i)}></div>
        })}
      </div>
    </div>
  );
